          ...
          location ~ "/2015-03-31/functions/(?:[^/])*/invocations" {
              content_by_lua_block {
                local function x()
                  ...
                  print(request_body)
                  local body_decoded = require("cjson").decode(request_body)
                  body_decoded["status_code"] = 201
                  local result_body = {test = "value"}
                  result_body["body"] = body_decoded["body"]
                  result_body["headers"] = body_decoded["headers"]
                  result_body["path"] = body_decoded["path"]
                  body_decoded["body"] = require("cjson").encode(result_body)
                  body_decoded["headers"] = {test = "x"}
                  body_decoded["headers"]["Content-Type"] = "application/json"
                  say(require("cjson").encode(body_decoded), 200)
                end
